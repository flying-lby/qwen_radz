...
def train():
    ...
    #     # 临时修改：无条件输出日志用于调试
    #     print(f"LOG: {msg}")
    #     if local_rank in [0, -1]:  # 只主进程打印
    #         logging.info(msg)

    # def save_with_log(obj, path, name=None):
    #     try:
    #         log(f"开始保存：{name or path}")
    #         t0 = time.time()
    #         torch.save(obj, path)
    #         log(f"保存完成：{name or path}，耗时：{time.time() - t0:.2f}s")
    #     except Exception as e:
    #         log(f"保存失败：{name or path}，异常：{e}")
    #         traceback.print_exc()

    # # ---- TRAIN ----
    # # 调试：检查 local_rank 的值
    # print(f"DEBUG: local_rank = {local_rank}, type = {type(local_rank)}")
    # try:
    #     log("准备开始训练...")
    #     if list(pathlib.Path(training_args.output_dir).glob("checkpoint-*")):
    #         log("检测到 checkpoint，恢复训练中...")
    #         trainer.train(resume_from_checkpoint=True)
    #     else:
    #         log("无 checkpoint，重新开始训练...")
    #         trainer.train()
    #     log("训练完成")
    # except Exception as e:
    #     log(f"训练过程发生异常：{e}")
    ...
    # try:
    #     if training_args.lora_enable:
    #         log("LoRA 模式启用，准备保存 LoRA 参数...")
    #         t0 = time.time()
    #         state_dict = get_peft_state_maybe_zero_3(model.named_parameters(), training_args.lora_bias)
    #         log(f"LoRA 参数获取完成，耗时：{time.time() - t0:.2f}s，参数数目：{len(state_dict)}")

    #         non_lora_state_dict = get_peft_state_non_lora_maybe_zero_3(model.named_parameters(), require_grad_only=True)
    #         log(f"非 LoRA 参数提取完成，参数数目：{len(non_lora_state_dict)}")
    ...
    #             save_with_log(non_lora_state_dict, os.path.join(training_args.output_dir, "non_lora_state_dict.bin"), "non_lora_state_dict")
    #             log("模型保存成功")
    #     else:
    #         log("非 LoRA 模式，调用 safe_save_model_for_hf_trainer")
